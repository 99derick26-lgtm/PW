-- scenes/squad.lua
-- Pixel War Online — Squad Scene
-- Shows conquered players, tax controls, conquer targets.
...
local function showTaxPopup(conquered)
    ...
    local panelW = SW - 40
    local panelH = 220
    local panel  = display.newRoundedRect(content, CX, CY, panelW, panelH, 16)
...
local function buildSquadGridTab(group)
    ...
    for i = 1, slots do
        ...
        if conquered then
            ...
            display.newText({
                parent=group, text="Contributions: "..tostring(conquered.contributionGold or 0).."g",
                x=cardX, y=cardY + 34,
            ...
            freeBtn:addEventListener("tap", function()
                local p = saveUtil.load()
                api.squad.liberate({
                    memberId = conquered.targetPlayerId or conquered.name,
                    name = conquered.name,
                }, function(response)
                    if response and response.ok and response.data then
                        local fresh = response.data.player or p
                        saveUtil.save(fresh)
                    else
                        squadUtil.removeConquered(p, conquered.name)
                        saveUtil.save(p)
    ...
    saveBtn:addEventListener("tap", function()
        ...
        for name, rate in pairs(pendingTaxRates) do
            if squadUtil.setTaxRate(p, name, rate) then
                ...
                api.squad.setTax({ name = name, rate = rate }, function(response)
...
function scene:show(event)
    ...
    if conquestResult then
        ...
        if conquestResult.won and conquestResult.target then
            ...
            if added then
                saveUtil.save(p)
                api.squad.recruit({ target = conquestResult.target }, function(response)
                    if response and response.ok and response.data and response.data.player then
                        saveUtil.save(response.data.player)
                    else
                        sync.pushPlayerSnapshot(p)
    ...
    api.squad.get(function(response)
        if response and response.ok and response.data then
            ...
            if (response.data.botGold or 0) > 0 then
                showToast("+"..tostring(response.data.botGold).."g from squad bots!", false)
            end
            rebuild()
        end
    end)
