...
local function buildSingleTab(group)
    ...
    display.newText({
        parent=group,
        text=state.joined and "STATUS: QUEUED FOR THE NEXT 64-PLAYER BRACKET" or "STATUS: NOT JOINED",
        x=CX, y=statusY, width=SW - 46,
        font=ui.FONT_BOLD, fontSize=10, align="center"
    }):setFillColor(state.joined and 0.45 or 0.90,
                    state.joined and 1.0 or 0.42,
                    state.joined and 0.55 or 0.30)

    display.newText({
        parent=group,
        text="This mode uses your leader only in 1v1 tournament battles.",
        x=CX, y=statusY + 26, width=SW - 46,
    ...
    btn:setFillColor(state.joined and 0.28 or 0.04,
                     state.joined and 0.08 or 0.18,
                     state.joined and 0.08 or 0.46, 0.97)
    btn.strokeWidth = 2
    btn:setStrokeColor(state.joined and 1.0 or 0.28,
                       state.joined and 0.24 or 0.68,
                       state.joined and 0.24 or 1.00, 0.88)
    display.newText({
        parent=group, text=state.joined and "LEAVE QUEUE" or "JOIN SINGLE TOURNAMENT",
        x=CX, y=btnY, font=ui.FONT_BOLD, fontSize=13
    }):setFillColor(1, 1, 1)
    btn:addEventListener("tap", function()
        local p = saveUtil.load()
        ensureTournamentState(p)
        p.tournaments.single.joined = not p.tournaments.single.joined
...
local function buildCrewTab(group)
    ...
    display.newText({
        parent=group,
        text=state.joined and "STATUS: CREW QUEUED FOR THE NEXT 64-CREW BRACKET" or "STATUS: NOT JOINED",
        x=CX, y=statusY, width=SW - 46,
        font=ui.FONT_BOLD, fontSize=10, align="center"
    }):setFillColor(state.joined and 0.45 or 0.90,
                    state.joined and 1.0 or 0.42,
    ...
    btn:setStrokeColor(state.joined and 1.0 or 0.28,
                       state.joined and 0.24 or 0.68,
                       state.joined and 0.24 or 1.00, 0.88)
    display.newText({ parent=group, text=state.joined and "LEAVE CREW QUEUE" or "JOIN CREW TOURNAMENT",
        x=CX, y=btnY, font=ui.FONT_BOLD, fontSize=13
    }):setFillColor(1, 1, 1)
    btn:addEventListener("tap", function()
        local p = saveUtil.load()
        ensureTournamentState(p)
        p.tournaments.crew.joined = not p.tournaments.crew.joined
